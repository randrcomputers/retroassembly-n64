        ...
        JSON.stringify(
          {
            platform,
            fileName: file.name,
            launchboxName: launchbox?.name || null,
            libretroName: libretro?.name || null,
            gameName: romData.gameName || null,
            gameReleaseYear: romData.gameReleaseYear || null,
            hasRawGameMetadata: !!romData.rawGameMetadata,
          },
          null,
          2,
        ),
...
export async function createRoms({ files, md5s, platform }: { files: File[]; md5s: string[]; platform: PlatformName }) {
  ...
  try {
    const identifyPayload = files.map((file, index) => ({
      md5: md5s[index] || '',
      name: file.name,
    }))

    console.log('[createRoms] identify start', {
      platform,
      files: identifyPayload,
    })
    ...
    console.log(
      '[createRoms] identify result',
      JSON.stringify(
        gameInfoList?.map((item, index) => ({
          index,
          fileName: files[index]?.name,
          launchboxName: item?.launchbox?.name || null,
          launchboxId: item?.launchbox?.databaseId || null,
          libretroName: item?.libretro?.name || null,
          libretroId: item?.libretro?.id || null,
          libretroPlatform: item?.libretro?.platform || null,
        })),
        null,
        2,
      ),
    ...
    console.warn('[createRoms] identify failed', error)
  }
