...
class MyClass {
    constructor() {
        ...
        this.flaData = null;
        this.dblist = [];
        this.pendingStateImport = null;
        this.pendingStateImportResolve = null;
        this.pendingStateImportReject = null;
        this.pendingStateImportPromise = null;
        this.stateRestoreInProgress = false;
        this.runtimeReady = false;
        this.emulatorStarted = false;
        this.windowFocusedBeforeBoot = false;
        this.windowFocusHandlerInstalled = false;
        window['Module'] = window['Module'] || {};
        window['Module']['canvas'] = document.getElementById('canvas');
        window['Module']['webglContextAttributes'] = {
            ...(window['Module']['webglContextAttributes'] || {}),
        ...
        this.embedMode = this.queryParams.get('embed') === '1';
        this.autostartFromQuery = this.queryParams.get('autostart') !== '0';
        this.initialRomUrl = this.queryParams.get('rom') || '';
        this.initialRomTitle = this.queryParams.get('title') || '';
        this.initialStateUrl = this.queryParams.get('state') || '';
        this.initialStateLoadRequested = false;

        if (this.embedMode) {
            document.body.classList.add('ra-embed-mode');
            this.rivetsData.showFPS = false;
            try { localStorage.setItem('n64wasm-showfps', 'false'); } catch (error) {}
        }
        ...
        if (this.initialRomUrl) {
            this.rivetsData.hasRoms = true;
            this.rivetsData.romList.push({
                url: this.initialRomUrl,
                title: this.initialRomTitle || this.extractRomName(this.initialRomUrl),
            });
        } else if (window["ROMLIST"].length > 0)
        {
            this.rivetsData.hasRoms = true;
    ...
    applyEmbedMode(){
        try {
            document.title = this.initialRomTitle ? `${this.initialRomTitle} - N64Wasm` : 'N64Wasm Embedded';
            const h1 = document.querySelector('#maindiv h1');
            if (h1 && this.initialRomTitle) h1.textContent = this.initialRomTitle;
            if (this.queryParams.get('hideMenu') === '1') {
            ...
            let style = document.getElementById('ra-embed-style');
            if (!style) {
                style = document.createElement('style');
                style.id = 'ra-embed-style';
                style.textContent = `
                    ...
                    #canvasDiv { position: absolute !important; inset: 0 !important; display: flex !important; align-items: center !important; justify-content: center !important; overflow: hidden !important; background: #000 !important; }
                    #canvasDiv canvas, #canvas { display: block !important; margin: 0 auto !important; }
                    #topPanel, #bottomPanel, #lblErrorOuter, #mobileBottomPanel, #mobileButtons { display: none !important; }
                `;
    ...
    installBridge(){
        ...
        if (!this.bridgeEscapeHandlerInstalled) {
            this.bridgeEscapeHandlerInstalled = true;
            document.addEventListener('keydown', (event) => {
                ...
                try {
                    window.parent?.postMessage({ type: 'ra-n64-escape' }, '*');
                } catch (error) {
                    console.log('[RA N64] escape bridge failed', error);
                }
            }, true);
        }

        window.N64WasmBridge = {
            isReady: () => !!window.Module,
            isRunning: () => !self.rivetsData.beforeEmulatorStarted,
            getRomName: () => self.rom_name,
            async loadRomUrl(url, title='') {
                self.initialRomUrl = url;
                self.initialRomTitle = title || self.extractRomName(url);
                self.rivetsData.romList = [{ url, title: self.initialRomTitle }];
                self.rivetsData.hasRoms = true;
                const selector = document.getElementById('romselect');
                if (selector) selector.value = url;
                await self.loadRom();
                return { ok: true, rom: self.rom_name };
            },
            async saveState() {
                return await self.exportStateFile();
            },
            async loadState(file) {
                return await self.importStateFile(file);
            },
            async exportState() {
                return await self.exportStateFile();
            },
        ...
        window.parent?.postMessage({ type: 'n64wasm-bridge-ready' }, '*');
    ...
    focusCanvas(force = false) {
        ...
        this._focusingCanvas = true;

        try {
            const canvas = document.getElementById('canvas') || document.querySelector('canvas');
            const canvasDiv = document.getElementById('canvasDiv');
            const target = canvasDiv || canvas;

            if (canvas && !canvas.hasAttribute('tabindex')) {
                canvas.setAttribute('tabindex', '0');
            }

            if (canvasDiv && !canvasDiv.hasAttribute('tabindex')) {
            ...
            this.windowFocusedBeforeBoot = true;
        } catch (error) {
            console.log('[RA N64] focusCanvas failed', error);
        } finally {
            setTimeout(() => {
                this._focusingCanvas = false;
            }, 0);
        }
    }

    installFocusGuards() {
        if (this.windowFocusHandlerInstalled) {
            return;
        }
        this.windowFocusHandlerInstalled = true;

        const refocus = () => this.focusCanvas();

        const bindFocus = () => {
            const canvas = document.getElementById('canvas') || document.querySelector('canvas');
            const canvasDiv = document.getElementById('canvasDiv');
            const targets = [canvasDiv, canvas].filter(Boolean);

            targets.forEach((target) => {
                if (!target || target.dataset.raFocusBound === '1') {
                    return;
                }
                ...
                target.dataset.raFocusBound = '1';

                ['pointerdown', 'mousedown', 'mouseup', 'click', 'touchstart'].forEach((evtName) => {
                    target.addEventListener(evtName, refocus, true);
                });
            });
        };

        bindFocus();
        document.addEventListener('DOMContentLoaded', bindFocus, { once: true });
        window.addEventListener('load', bindFocus, { once: true });
    ...
    scheduleAutoStart() {
        ...
        if (!this.initialRomUrl || !this.autostartFromQuery) {
            return;
        }
        ...
        const tryStart = () => {
            ...
            const selector = document.getElementById('romselect');
            if (selector && self.initialRomUrl) {
                selector.value = self.initialRomUrl;
            }

            console.log('[RA N64] autostart check', {
                attempt: attempts,
                initialRomUrl: self.initialRomUrl,
                moduleInitializing: self.rivetsData.moduleInitializing,
                hasLoadRom: typeof self.loadRom,
            });

            if (typeof self.loadRom === 'function' && !self.rivetsData.moduleInitializing) {
                console.log('[RA N64] autostart calling loadRom()');
                self.loadRom();
                return;
            }
            ...
            if (attempts < maxAttempts) {
                ...
                console.log('[RA N64] autostart failed after max attempts');
            }
        };
    ...
    async exportStateFile(){
        return new Promise((resolve, reject) => {
            try {
                this.pendingStateExportResolve = resolve;
                this.pendingStateExportReject = reject;
                Module._neil_serialize();
            } catch (error) {
                this.pendingStateExportResolve = null;
                this.pendingStateExportReject = null;
                reject(error);
            }
        });
    }

    async captureScreenshotFile(){
        const canvas = document.getElementById('canvas') || document.querySelector('canvas');
        if (!canvas) {
    ...
    async importStateFile(file){
        try {
            const arrayBuffer = await file.arrayBuffer();
            const byteArray = new Uint8Array(arrayBuffer);

            const mod = window.Module || {};
            const fsApi = mod.FS || window.FS;
            const runtimeReadyNow = !this.rivetsData.moduleInitializing && this.runtimeReady && this.emulatorStarted && fsApi && typeof fsApi.writeFile === 'function' && typeof mod._neil_unserialize === 'function';

            if (!runtimeReadyNow) {
                const alreadyQueued = !!this.pendingStateImportPromise;
                this.pendingStateImport = byteArray;

                if (!alreadyQueued) {
                    console.log('[RA N64] runtime not ready, queueing state import', {
                        moduleInitializing: this.rivetsData.moduleInitializing,
                        runtimeReady: this.runtimeReady,
                        emulatorStarted: this.emulatorStarted,
                        hasFS: !!fsApi,
                        hasWriteFile: !!(fsApi && fsApi.writeFile),
                        size: byteArray.length,
                    });

                    this.pendingStateImportPromise = new Promise((resolve, reject) => {
                        this.pendingStateImportResolve = resolve;
                        this.pendingStateImportReject = reject;
                    });
                }

                return this.pendingStateImportPromise;
            }

            fsApi.writeFile('/savestate.gz', byteArray);
            mod._neil_unserialize();
            return { ok: true, size: byteArray.length };
        } catch (error) {
            console.log('importStateFile failed', error);
            throw error;
        }
    }

    flushPendingStateImport(){
        try {
            if (!this.pendingStateImport || this.stateRestoreInProgress) {
                return;
            }
            ...
            if (!this.runtimeReady || !this.emulatorStarted || !fsApi || typeof fsApi.writeFile !== 'function') {
                return;
            }

            if (typeof mod._neil_unserialize !== 'function') {
                return;
            }

            this.stateRestoreInProgress = true;
            const byteArray = this.pendingStateImport;
            this.pendingStateImport = null;

            fsApi.writeFile('/savestate.gz', byteArray);
            mod._neil_unserialize();

            console.log('[RA N64] queued state restored after boot', { size: byteArray.length });

            if (this.pendingStateImportResolve) {
                this.pendingStateImportResolve({ ok: true, size: byteArray.length, deferred: true });
            }
        } catch (error) {
            console.log('[RA N64] flushPendingStateImport failed', error);

            if (this.pendingStateImportReject) {
                this.pendingStateImportReject(error);
            }
        } finally {
            this.pendingStateImportResolve = null;
            this.pendingStateImportReject = null;
    ...
    processPrintStatement(text) {
        ...
        if (text.includes('mupen64plus: Starting R4300 emulator: Cached Interpreter')) {
            ...
            myClass.emulatorStarted = true;

            setTimeout(() => {
                myClass.flushPendingStateImport();
            }, 1500);

            if (myClass.initialStateUrl && !myClass.initialStateLoadRequested) {
                myClass.initialStateLoadRequested = true;
                setTimeout(async () => {
                    try {
                        const response = await fetch(myClass.initialStateUrl);
                        if (!response.ok) {
                            throw new Error('Failed to fetch startup state: ' + response.status);
                        }
                        const blob = await response.blob();
                        const file = new File([blob], 'startup.state', { type: blob.type || 'application/octet-stream' });
                        await myClass.importStateFile(file);
                        console.log('[RA N64] startup state loaded');
                    } catch (error) {
                        console.log('[RA N64] startup state load failed', error);
                    }
                }, 1600);
            }
    ...
    async LoadEmulator(byteArray){
        ...
        else
        {
            ...
            if (this.embedMode && this.toggleFPSModule) {
                try { this.toggleFPSModule(0); } catch (error) {}
            }
            this.applyEmbedCanvasLayout();
        }
    ...
    async initModule(){
        ...
        myClass.runtimeReady = true;
        myClass.rivetsData.moduleInitializing = false;
        // Wait for a real running emulator session before restoring a queued state.
        //myClass.loadFiles();
    }
    ...
    loadFromDatabase() {
        ...
        request.onsuccess = function (ev) {
            ...
            rom.onsuccess = function (event) {
                ...
                if (!myClass.runtimeReady || !myClass.emulatorStarted || !fsApi || typeof fsApi.writeFile !== 'function' || typeof mod._neil_unserialize !== 'function') {
                    console.log('[RA N64] local state load deferred - runtime/emulator not ready');
                    myClass.pendingStateImport = byteArray;
                    return;
                }
    ...
    SaveStateEvent()
    {
        ...
        if (myClass.pendingStateExportResolve) {
            const file = new File([compressed], (myClass.rom_name || 'state') + '.n64wasm', { type: 'application/octet-stream' });
            myClass.pendingStateExportResolve(file);
            myClass.pendingStateExportResolve = null;
            myClass.pendingStateExportReject = null;
        }
        ...
        oReq.onload = function (oEvent) {
            ...
            try{
                if (arrayBuffer) {
                    ...
                    if (!myClass.runtimeReady || !myClass.emulatorStarted || !fsApi || typeof fsApi.writeFile !== 'function' || typeof mod._neil_unserialize !== 'function') {
                        console.log('[RA N64] cloud load deferred - runtime/emulator not ready');
                        myClass.pendingStateImport = byteArray;
                        return;
                    }
    ...
    retrieveSettings(){
        ...
        if (this.embedMode) this.rivetsData.showFPS = false;
...
window["Module"] = window["Module"] || {};
window["Module"].onRuntimeInitialized = myClass.initModule;
window["Module"].canvas = document.getElementById('canvas');
window["Module"].print = (text) => myClass.processPrintStatement(text);
// window["Module"].printErr = (text) => myClass.print(text);
